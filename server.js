...
// video route
app.use(_.get('/video/:id', async ctx => {
    accept(ctx);
    id = Number(id);
    let result = await video.read(id, false);
    ctx.body = result;
}));
app.use(_.get('/video/:id/init', async ctx => {
    accept(ctx);
    id = Number(id);
...
app.use(_.get('/video/:id/add/:time', async ctx => {
    accept(ctx);
    id = Number(id);
    time = String(time).endsWith(" ") ?
        String(time).substring(0, String(time).length - 1) :
        String(time);
    let result = await video.add(id, type);
    ctx.body = result;
}));

app.use(_.get('/video/:id/remove', async ctx => {
    accept(ctx);
    id = Number(id);
...
app.use(_.get('/video/:id/update/:time', async ctx => {
    accept(ctx);
    id = Number(id);
    time = String(time).endsWith(" ") ?
        String(time).substring(0, String(time).length - 1) :
