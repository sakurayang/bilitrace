...
const accept = ctx => {
	...
	ctx.set({
		...
		"Access-Control-Allow-Headers":
			"cache-control,content-type,hash-referer,x-requested-with"
	});
};
...
app.use(
	_.get("/video/add", async ctx => {
		accept(ctx);
		let query = ctx.query;
		if ("id" in query) {
			let id = String(query.id).startsWith("BV")
				? require("./utils/video/idConverter").bv2av(query.id)
				: Number(query.id);
			let time =
				"time" in query
					? String(query.time).endsWith(" ")
						? String(query.time).substring(
								0,
								String(query.time).length - 1
						  )
						: String(query.time)
					: "*/5 * * * *";
...
app.use(
	_.get("/live/status", async ctx => {
		...
		ctx.body = JSON.parse(
			await request(
				`https://api.live.bilibili.com/room/v1/Room/room_init?id=${query.id}`
			)
		).data.live_status;
	})
);
