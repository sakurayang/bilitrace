 ...
 *
 * @param {Buffer} packet
 * @param {JSON[]} msg_array
 * @return {Array}
 */
function parsePacket(packet, msg_array = []) {
	...
	if (
		...
		// length error, but try to parse it
		packet = packet.writeInt32BE(packet.length, 0);
		return parsePacket(Buffer.from(packet).slice(data_length), msg_array);
	} else if (data_length < packet.length) {
		// multi msg
		let data = parseBody(
			operation,
			protocol,
			Buffer.from(packet).slice(16, data_length)
		);
		msg_array.push(data);
		return parsePacket(Buffer.from(packet).slice(data_length), msg_array);
	} else if (data_length == packet.length) {
		...
		let data = parseBody(
			operation,
			protocol,
			Buffer.from(packet).slice(16)
		);
		msg_array.push(data);
		return msg_array;
	} else {
		msg_array.push({ code: -10086, msg: "unknown error" });
		return msg_array;
	}
}
